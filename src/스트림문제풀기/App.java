...
//		.forEach(oc -> System.out.println(oc.getId()));
		...
		List<List<OnlineClass>> jihyunEvents = new ArrayList<>();
		jihyunEvents.add(springClasses);
		jihyunEvents.add(javaClasses);
		
		System.out.println("두 수업 목록에 들어있는 모든 수업 아이디 출력");
		
		jihyunEvents.stream().flatMap(Collection::stream)
		.forEach(oc -> System.out.println(oc.getId()));
...
//		.map(oc -> oc.getTitle());
		...
		//오퍼레이터들은 순서에 따라 다음 오퍼레이터의 값이 바뀔수도 있음.
	}
